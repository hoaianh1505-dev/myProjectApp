...
import { createProduct, getProductById, handleDeleteProduct, postUpdateProduct as updateProductService } from "src/services/admin/product.service";
...
const postUpdateProduct = async (req: Request, res: Response) => {
    ...
    if (!validate.success) {
        const errorsZod = validate.error.issues;
        const errors = errorsZod.map(item => `${item.message} (${item.path[0]})`);
        const product = { id, name, price, detailDesc, shortDesc, quantity, factory, target };

        return res.render("admin/product/detail.ejs", { product, errors, factoryOptions, targetOptions });
    }

    const image = req?.file?.filename ?? null;
    await updateProductService(+id, name, +price, detailDesc, shortDesc, +quantity, factory, target, image);
